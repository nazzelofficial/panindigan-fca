...
var TypedEventEmitter = class extends EventEmitter3 {
};
...
// src/config/index.ts
expand(dotenvConfig());
var stealthLevelSchema = z.enum(["off", "low", "medium", "high", "paranoid"]);
var storageAdapterSchema = z.enum(["memory", "file", "libsql", "redis"]);
...
var configSchema = z.object({
  ...
  http: z.object({
    ...
    timeout: z.object({
      connect: z.number().int().min(100).default(5e3),
      request: z.number().int().min(1e3).default(3e4),
      body: z.number().int().min(1e3).default(6e4)
    }).default(() => ({})),
    retries: z.object({
  ...
  mqtt: z.object({
    reconnect: z.object({
      ...
      baseDelay: z.number().int().min(100).default(1e3)
    }).default(() => ({})),
    heartbeat: z.object({
      interval: z.number().int().min(5e3).default(6e4)
    }).default(() => ({}))
  }).default(() => ({})),
  cache: z.object({
  ...
  stealth: z.object({
    ...
    delays: z.object({
      ...
      actionDelay: z.object({ min: z.number().default(300), max: z.number().default(1800) }).default(() => ({})),
      messageDelay: z.object({ min: z.number().default(800), max: z.number().default(4e3) }).default(() => ({})),
      paginationDelay: z.object({ min: z.number().default(200), max: z.number().default(900) }).default(() => ({}))
    }).default(() => ({})),
    typingSimulation: z.object({
      enabled: z.boolean().default(true),
      wpm: z.object({ min: z.number().default(40), max: z.number().default(80) }).default(() => ({})),
  ...
  refresh: z.object({
    checkInterval: z.number().int().min(6e4).default(3e5),
    threshold: z.number().int().min(6e4).default(18e5),
  ...
  keepalive: z.object({
    enabled: z.boolean().default(true),
    interval: z.number().int().min(6e4).default(6e5),
...
// src/http/index.ts
import { Pool, fetch as undicicFetch } from "undici";
...
var MqttClient = class {
  constructor(jar, userId, emitter, config, logger, onPresenceUpdate, wsAgent) {
    ...
    this.clientId = `mqttwsclient_${userId}_${randomBytes2(4).toString("hex")}`;
        ...
        for (const pkt of packets) {
          switch (pkt.type) {
            case MQTT_CONNACK:
              ...
              if (!this.mqttSid) this.mqttSid = randomBytes2(8).toString("hex");
              this.startPing();
  ...
  async heapSnapshot(outputPath) {
    ...
    return new Promise((resolve, reject) => {
      ...
      session.post("HeapProfiler.takeHeapSnapshot", { reportProgress: false }, (err) => {
        ...
        writeFile2(outputPath, chunks.join(""), "utf8").then(resolve).catch(reject);
      });
    });
  }
  async healthCheck() {
...
var IdSchema = z2.union([z2.string(), z2.number()]).transform((v) => String(v));
var TimestampSchema = z2.union([z2.string(), z2.number()]).transform((v) => {
  if (typeof v === "string") {
    const n = Number(v);
    if (!Number.isNaN(n)) {
...
var AttachmentSchema = z2.object({
  ...
  type: z2.string().optional().default("unknown"),
  url: z2.string().optional(),
  name: z2.string().optional(),
  size: z2.number().optional()
});
var MessageNodeSchema = z2.object({
  message_id: IdSchema.optional(),
  id: IdSchema.optional(),
  timestamp_precise: TimestampSchema.optional(),
  timestamp: TimestampSchema.optional(),
  message: z2.object({ text: z2.string().nullable().optional() }).nullable().optional(),
  message_sender: z2.object({
    id: IdSchema.optional(),
    name: z2.string().optional()
  }).optional(),
  blob_attachments: z2.array(AttachmentSchema).optional(),
  sticker: z2.object({ id: IdSchema.optional(), label: z2.string().optional() }).nullable().optional()
});
var MessageListResponseSchema = z2.object({
  data: z2.object({
    viewer: z2.object({
      message_thread: z2.object({
        thread_key: z2.object({ thread_fbid: IdSchema.optional() }).optional(),
        messages: z2.object({
          edges: z2.array(
            z2.object({
          ...
          page_info: z2.object({
            has_previous_page: z2.boolean().optional().default(false),
            start_cursor: z2.string().nullable().optional()
          })
        })
      }).optional()
    }).optional()
...
var SendMessageResponseSchema = z2.object({
  ...
  error: z2.number().optional(),
  errorSummary: z2.string().optional()
}).passthrough();
var ParticipantSchema = z2.object({
  id: IdSchema,
  name: z2.string().optional().default(""),
  profile_picture: z2.object({ uri: z2.string().optional() }).nullable().optional()
});
var ThreadNodeSchema = z2.object({
  thread_key: z2.object({ thread_fbid: IdSchema.optional(), other_user_id: IdSchema.optional() }).optional(),
  id: IdSchema.optional(),
  name: z2.string().nullable().optional(),
  image: z2.object({ uri: z2.string() }).nullable().optional(),
  is_group_thread: z2.boolean().optional().default(false),
  all_participants: z2.object({
    edges: z2.array(
      z2.object({ node: ParticipantSchema })
    ).optional().default([])
  }).optional(),
  unread_count: z2.number().optional().default(0),
  mute_until: TimestampSchema.optional(),
  folder: z2.string().optional(),
  updated_time_precise: TimestampSchema.optional(),
  last_message: z2.object({
    nodes: z2.array(MessageNodeSchema).optional()
  }).optional()
});
var ThreadEdgeSchema = z2.object({ node: ThreadNodeSchema });
var ThreadListResponseSchema = z2.object({
  data: z2.object({
    // Connection may be under viewer.message_threads, user.message_threads,
    // or the top-level message_threads key — accept any.
    viewer: z2.object({
      message_threads: z2.object({
        edges: z2.array(ThreadEdgeSchema).optional().default([]),
        page_info: z2.object({
          has_next_page: z2.boolean().optional().default(false),
          end_cursor: z2.string().nullable().optional()
        }).optional()
      }).optional(),
      threads: z2.object({
        edges: z2.array(ThreadEdgeSchema).optional().default([]),
...
var UserProfileSchema = z2.object({
  ...
  profile_picture: z2.object({ uri: z2.string().optional() }).nullable().optional(),
  friends_count: z2.number().nullable().optional(),
  mutual_friends: z2.object({ count: z2.number().optional() }).nullable().optional(),
  is_friend: z2.boolean().optional()
});
var UserProfileResponseSchema = z2.object({
  data: z2.object({
    user: UserProfileSchema.optional(),
    viewer: z2.object({ actor: UserProfileSchema.optional() }).optional()
  }).optional()
}).passthrough();
var FriendListResponseSchema = z2.object({
  data: z2.object({
    viewer: z2.object({
      friends: z2.object({
        edges: z2.array(
          z2.object({ node: UserProfileSchema })
        ).optional().default([]),
        page_info: z2.object({
          has_next_page: z2.boolean().optional().default(false),
          end_cursor: z2.string().nullable().optional()
...
var PresenceEntrySchema = z2.object({
  user_id: IdSchema.optional(),
  // Facebook uses is_online or is_present depending on API version
  is_online: z2.boolean().optional(),
  is_present: z2.boolean().optional(),
  is_active: z2.boolean().optional(),
  // Seconds-precision Unix timestamp — last_active_time or last_active
  last_active_time: z2.union([z2.string(), z2.number()]).optional(),
  last_active: z2.union([z2.string(), z2.number()]).optional()
});
var PresenceResponseSchema = z2.object({
  data: z2.object({
    user: z2.object({
...
var PollOptionSchema = z2.object({
  id: IdSchema.optional(),
  text: z2.string(),
  vote_count: z2.number().optional().default(0),
  voters: z2.array(IdSchema).optional().default([])
});
var PollResponseSchema = z2.object({
  data: z2.object({
    poll: z2.object({
      id: IdSchema.optional(),
      title: z2.string().optional(),
      options: z2.array(PollOptionSchema).optional().default([]),
      total_vote_count: z2.number().optional().default(0),
      expiration_time: TimestampSchema.optional()
    }).optional()
  }).optional()
}).passthrough();
var MessageSearchNodeSchema = z2.object({
  message_id: IdSchema.optional(),
  id: IdSchema.optional(),
  thread_key: z2.object({ thread_fbid: IdSchema.optional() }).optional(),
  // Sender info appears on message search nodes
  sender: z2.object({ id: IdSchema.optional(), name: z2.string().optional() }).optional(),
  text: z2.string().optional(),
  snippet: z2.string().optional(),
  timestamp: z2.union([z2.string(), z2.number()]).optional()
});
var MessageSearchResponseSchema = z2.object({
  data: z2.object({
    // Both message and thread searches use the search_results key
    search_results: z2.object({
      edges: z2.array(
        z2.object({ node: MessageSearchNodeSchema })
      ).optional().default([]),
      page_info: z2.object({
        has_next_page: z2.boolean().optional().default(false),
        end_cursor: z2.string().nullable().optional()
...
var ThreadSearchNodeSchema = z2.object({
  id: IdSchema.optional(),
  thread_key: z2.union([IdSchema, z2.object({ thread_fbid: IdSchema.optional() })]).optional(),
  name: z2.string().nullable().optional(),
  is_group_thread: z2.boolean().optional().default(false),
  all_participants: z2.array(
    z2.union([
      z2.object({ node: z2.object({ name: z2.string().optional() }).passthrough() }),
      z2.object({ name: z2.string().optional() }).passthrough()
    ])
  ).optional(),
  last_message: z2.object({ timestamp: z2.union([z2.string(), z2.number()]).optional() }).optional()
});
var ThreadSearchResponseSchema = z2.object({
  data: z2.object({
    search_results: z2.object({
      edges: z2.array(
        z2.object({ node: ThreadSearchNodeSchema })
      ).optional().default([]),
      page_info: z2.object({
        has_next_page: z2.boolean().optional().default(false),
        end_cursor: z2.string().nullable().optional()
...
var UploadResponseSchema = z2.object({
  payload: z2.object({
    metadata: z2.array(
      z2.object({
        fbid: z2.union([z2.string(), z2.number()]).optional(),
        filename: z2.string().optional(),
        filetype: z2.string().optional(),
        attachment_id: z2.union([z2.string(), z2.number()]).optional()
      })
    ).optional(),
    fbid: z2.union([z2.string(), z2.number()]).optional(),
    attachment_id: z2.union([z2.string(), z2.number()]).optional(),
    attachment_token: z2.string().optional()
  }).optional()
}).passthrough();
...
var LoginResponseSchema = z2.object({
  jsmods: z2.object({
    require: z2.array(z2.unknown()).optional()
  }).optional(),
  error: z2.number().optional(),
  errorSummary: z2.string().optional(),
  errorDescription: z2.string().optional()
}).passthrough();
...
export {
  ...
  UserProfileSchema,
  UsersModule,
  buildFormRequest3 as buildFormRequestBody,
  buildGraphQLBody,
  buildGraphQLRequest2 as buildGraphQLRequestBody,
  buildJsonBody,
  buildLightspeedBody,
  buildMultipartBody,
  buildStealthHeaders,
