...
class SecurityGuard {
    constructor(options) {
        ...
        this.COOLDOWN_DURATION = 120000; // 2 minutes
    }
    async checkRateLimits() {
        if (!this.options.enable || !this.options.smartRateLimiting)
            return;
        const now = Date.now();
    ...
    async handleCheckpoint(body, pageUrl = 'https://www.facebook.com/') {
        if (!this.options.checkpointSolver)
            return false;
        // Detect checkpoint
    ...
    async solveCaptcha(siteKey, pageUrl) {
        ...
        try {
            if (provider === '2captcha') {
                ...
                const res = await axios_1.default.post(`http://2captcha.com/in.php?key=${apiKey}&method=userrecaptcha&googlekey=${siteKey}&pageurl=${pageUrl}&json=1`);
                ...
                while (tries < 30) {
                    ...
                    const poll = await axios_1.default.get(`http://2captcha.com/res.php?key=${apiKey}&action=get&id=${id}&json=1`);
                ...
                // 1. Create task
                const res = await axios_1.default.post('https://api.anti-captcha.com/createTask', {
                    clientKey: apiKey,
                    task: {
                ...
                while (tries < 30) {
                    await new Promise(r => setTimeout(r, 5000));
                    const poll = await axios_1.default.post('https://api.anti-captcha.com/getTaskResult', {
                        clientKey: apiKey,
                        taskId: taskId
                    });
                    if (poll.data.errorId !== 0)
                        throw new Error(poll.data.errorDescription);
