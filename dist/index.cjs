...
var HttpClient = class {
  constructor(jar, config, stealth, pipeline2, logger, emitter) {
    ...
    this.pool = new import_undici2.Pool(FB_BASE_URL, {
  ...
  async request(options) {
    ...
    const attempt = async () => {
      ...
      let resp;
      try {
        resp = await (0, import_undici2.fetch)(reqCtx.url, {
...
var FilesModule = class {
  ...
  async download(url, options) {
    ...
    const writeStream = (0, import_node_fs2.createWriteStream)(options.destination);
    let bytesWritten = 0;
    try {
